...
class CsvApi(Api):
    ...
    def write(self,
        ...
        with open(filename, 'w') as dst:
            dst.write('RED, Red HP, Red score, Red warnings, Blue warnings, Blue score, Blue HP, BLUE\n')
            for p in pairs:
                dst.write(p.fighter_1 + ',' + str(fighters[p.fighter_1].rating)  # Red name & HP
                          + ', , ' + str(fighters[p.fighter_1].warnings)  # Red score and warnings
                          + ', ' + str(fighters[p.fighter_2].warnings) + ', '  # Blue warnings and score
    ...
    def parse_results(line):
        ...
        warnings_1 = int(split[3].rstrip().strip('\"'))
        fighter_2 = split[7].rstrip().strip('\"')
        result_2 = -abs(int(split[5].rstrip().strip('\"')))
        warnings_2 = int(split[4].rstrip().strip('\"'))

        r = Round(status='finished', score_1=result_1, score_2=result_2, warnings_1=warnings_1, warnings_2=warnings_2)
